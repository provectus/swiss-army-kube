...
variable "argocd" {
  default = {
    repository = "argo-infra"
    branch     = "main"
    owner      = "sergeyhovakimyan"
  }
  type        = map(string)
...
variable "karpenter_mode" {
  type        = string
  description = "karpenter_mode is define where karpenter is located : FARGATE-PROFILE or EKS-MANAGED-NODE-GROUP"
  default     = "FARGATE-PROFILE"
  validation {
    condition     = contains(["FARGATE-PROFILE", "EKS-MANAGED-NODE-GROUP"], var.karpenter_mode)
    error_message = "Valid value is one of the following: FARGATE-PROFILE, EKS-MANAGED-NODE-GROUP."
  }
}
